...
Reddit / WallStreetBets rule: retail attention is useful only as a sentiment and crowding input. The script filters generic daily threads, portfolio screenshots, and long ticker dumps before evaluating tickers. Prefer the official OAuth API when credentials are configured. Without OAuth, the refresh uses one reduced WallStreetBets `hot` RSS sample instead of repeatedly calling public JSON or several subreddit feeds that hosted runners commonly rate-limit. RSS has no vote or comment totals, so describe its output only as unranked recent post mentions, never as trending or concentrated attention. If a last-good Reddit cache is used, label it as cached sentiment and include the age. If no clean signal remains, the briefing should explicitly say the Reddit input was not clean enough to use, not invent a takeaway.
